...
//  "clear":        { "value": WeakMap_clear    }, // WeakMap#clear():undefined
});
...
function Iterator_values() {
    var index = ++this._index;
    var done = index >= this._data.length;

    return {
        "value": done ? undefined : this._data[index],
        "done":  done
    };
}

function Iterator_keyAndValues() {
    var index = ++this._index;
    var done = index >= this._data.length;

    return {
        "value": done ? undefined : [ index, this._data[index] ],
        "done":  done
    };
}
...
function Array_entries() { // @ret KeyValueIterator
    ...
    return new Iterator(this, Iterator_keyAndValues);
}
...
function Array_keys() { // @ret KeyIterator
    ...
    return new Iterator(this, Iterator_keys);
}
...
function Array_values() { // @ret ValueIterator
    ...
    return new Iterator(this, Iterator_values);
}
