...
function ES(version) { // @arg Number(= 5.0): polyfill version number.
    ...
    switch (version) {
    case 6.0: _polyfill(ES6_POLYFILL);
    case 5.1:
    case 5.0: _polyfill(ES5_POLYFILL);
        break;
    }
...
function _polyfill(obj) {
    for (var nativeClassName in obj) { // "Object", "Array", "String", ...
        nativeClassName in ES || (ES[nativeClassName] = {});

        for (var propertyName in obj[nativeClassName]) { // "isArray", "prototype", ...
            if (propertyName === "prototype") {
                propertyName in ES[nativeClassName] || (ES[nativeClassName][propertyName] = {});
                ...
                for (var methodName in prototype) { // "indexOf", ...
                    ...
                    _extends(ES[nativeClassName]["prototype"], methodName, prototype[methodName]);
                }
                ...
                _extends(ES[nativeClassName], propertyName, value);
            }
        }
    }
}
...
function Array_of(ooo) { // @var_args Any: values
                         // @ret AnyArray:
...
function Array_from(fakeArray) { // @arg FakeArray: Arguments or NodeList
                                 ...
                                 // @help: Array.from
//{@assert
    _if(arguments.length !== 1, "This version Array.from did not support the 2nd and 3rd arguments.");
//}@assert
...
//}@node
global["ES"] ? (global["ES_"] = ES) // already exsists
             : (global["ES"]  = ES);
